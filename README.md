...
Advanced feature in Process data:

GenomeBridge provides specialized dataset generation tools for common genomic machine learning tasks:

**Promoter Region Dataset**: Generate promoter vs. non-promoter classification data from EPDnew database (hg38, mm10, danRer11)
```bash
genomebridge-cli process_promoter genomeBridge/Examples/process_promoter.yaml
```
...
```bash
genomebridge-cli process_emp genomeBridge/Examples/process_emp.yaml
```

**Enhancer Region Dataset**: Build enhancer vs. non-enhancer classification data from FANTOM5 annotations (hg38, mm10)
```bash
genomebridge-cli process_enhancer genomeBridge/Examples/process_enhancer.yaml
```
...
GenomeBridge provides comprehensive tools for understanding and interpreting genomic foundation models through sparse autoencoder (SAE) interpretation to provide deep insights into model behavior and biological significance.
...
```bash
genomebridge-cli sae_train genomeBridge/Examples/sae_train.yaml
```
...
```bash
genomebridge-cli sae_train genomeBridge/Examples/sae_regression.yaml
```
...
You can run protein generation using the following command:

```bash
genomebridge-cli protein genomeBridge/Examples/protein_generation.yaml
```

### ⚙️ Configuration

You can specify the following parameters in the YAML file:

```yaml
model_name_or_path: <MODEL_NAME>  # Model to use for generation
gen_id: <GENOME_ID>        # Target genome identifier
start: <START_POSITION>    # Start genomic coordinate
end: <END_POSITION>        # End genomic coordinate
strand: <STRAND>           # DNA strand (-1 or 1)
prompt_start: <PROMPT_START>     # Prompt region start
prompt_end: <PROMPT_END>         # Prompt region end
structure_start: <STRUCTURE_START>   # Structure constraint start
structure_end: <STRUCTURE_END>       # Structure constraint end
num: <NUM_SEQUENCES>       # Number of sequences to generate
min_seq_len: <MIN_LENGTH>  # Minimum sequence length
max_seq_len: <MAX_LENGTH>  # Maximum sequence length
foldmason_path: <PATH_TO_FOLDMASON>  # FoldMason executable path
output_prefix: <OUTPUT_PREFIX>       # Output file prefix
```