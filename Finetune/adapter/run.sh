...
# Embedding for bacteria
TYPE=GO
MODEL_PATH=/global/cfs/cdirs/jgirnd/projects/LLMs/genomeocean/all_checkpoints/mistral_4B_meta55k_hmp4k_seq10240/checkpoint-1600 
for RESO in 10240; do
    for MAX_LEN in 10240; do
        for DATA in genome_bac; do
            for NAME in test; do
                python generate_embedding.py \
                  --type $TYPE \
                  --model_tokenizer_path $MODEL_PATH \
...
for RESO in 10240; do
    for MAX_LEN in 10240; do
        CUDA_VISIBLE_DEVICES=3 torchrun --nproc-per-node=1 --master-port 2011 main.py \
            --model_type $MODEL \
            --label_dir ./data/$DATA \
...
MAX_LEN=10240
for TYPE in BPE; do
    CUDA_VISIBLE_DEVICES=0 torchrun --nproc-per-node=1 --master-port 2014 main.py \
        --model_type $MODEL \
        --label_dir ./data/$DATA \
...
MAX_LEN=10240
for TYPE in TNF; do
    CUDA_VISIBLE_DEVICES=1 torchrun --nproc-per-node=1 --master-port 2012 main.py \
        --model_type $MODEL \
        --label_dir ./data/$DATA \
        --data_dir ./embedding/${TYPE}_embeddings/$DATA \
        --feat_dim 256 \
