...
# Define regression metrics using Mean Squared Error
def compute_metrics(eval_pred):
    predictions, labels = eval_pred
    # Squeeze predictions in case of extra dimensions
    if isinstance(predictions, tuple):  # Unpack logits if it's a tuple
...
def preprocess_logits_for_metrics(logits, labels):
    # For regression, no preprocessing is needed
    return logits
...
class ModelArguments:
    ...
    # Removed quantization options as they are not needed for regression
    use_fp16: bool = field(default=False, metadata={"help": "Use FP16 precision."})
