...
def router_z_loss_func(concatenated_gate_logits: torch.Tensor) -> float:
    r"""
    ...
    """
    num_tokens, _ = concatenated_gate_logits.shape
...
def load_balancing_loss_func(
    concatenated_gate_logits: torch.Tensor, num_experts: torch.Tensor = None, top_k=2, attention_mask: Optional[torch.Tensor] = None
) -> float:
    r"""
...
class MixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        if output_router_logits:
            ...
            z_loss = self.router_z_loss_coef * router_z_loss_func(concatenated_gate_logits).to(loss.device)
...
class MixtralPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MixtralLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = MixtralPredictionHeadTransform(config)
...
class MixtralOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = MixtralLMPredictionHead(config)
...
class MixtralForMaskedLM(MixtralPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `MixtralForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.model = MixtralModel(config)
        self.cls = MixtralOnlyMLMHead(config)
