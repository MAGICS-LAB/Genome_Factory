    ...
    def __init__(self, 
        ...
        try:
            ...
            if 'sequence' in df_cleaned.columns and 'label' in df_cleaned.columns:
                # Genomic data format: sequence, label (continuous for regression)
                data = [[row['sequence'], float(row['label'])] for _, row in df_cleaned.iterrows()]
        ...
        if len(data[0]) == 2:
            ...
            labels = [float(d[1]) for d in data]  # Use float for regression
        elif len(data[0]) == 3:
            # data is in the format of [text1, text2, label]
            logging.warning("Perform sequence-pair classification...")
            texts = [[d[0], d[1]] for d in data]
            labels = [float(d[2]) for d in data]  # Use float for regression
        else:
        ...
        self.labels = labels
        self.num_labels = 1  # Fixed to 1 for regression
