...
class GenomeDataset:
    ...
    def basic_quality_filter(self, df):
        MIN_LEN = 50
        MAX_LEN = 1000
        GC_LOW = 0.3
        GC_HIGH = 0.7
        ...
        filtered['gc_content'] = filtered['sequence'].apply(self.gc_content)
        filtered['length'] = filtered['sequence'].apply(len)
        return filtered

    def advanced_statistical_qc(self, df):
        AMBIG_N_THRESHOLD = 0.05
        RARE_PROFILE_THRESHOLD = 0.01
    ...
    def run_data_cleaning_pipeline(self, csv_path):
        df = pd.read_csv(csv_path)
        print(f"Original dataset size: {len(df)}")

        df = self.basic_quality_filter(df)
        print(f"After basic filtering: {len(df)}")

        df = self.advanced_statistical_qc(df)
        print(f"After advanced statistical QC: {len(df)}")

        df_validated, validation_results = self.final_validation(df)
